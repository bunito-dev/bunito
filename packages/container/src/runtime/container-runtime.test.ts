...
function Plugin(options?: Parameters<typeof createExtensionDecorator>[1]) {
  return createExtensionDecorator(Plugin, options);
}
...
describe('ContainerRuntime', () => {
  it('resolves values, classes, factories, injections, indexed providers, and lifecycle hooks', async () => {
    ...
    expect((missingProviderError as Error).message).toContain('was not found');
    ...
    expect((unavailableProviderError as Error).message).toContain(
      'is not available in module',
    );

    let missingInjectionError: unknown;
    try {
      await runtime.resolveInjections([{ useToken: 'missing-required' }]);
    } catch (error) {
  ...
  it('falls back to onResolve when onInit is absent and guards disposable lifecycle handlers', async () => {
    @Provider()
    ...
    expect((secondDestroyError as Error).message).toContain('can only be called once');
  });
  ...
  it('returns an empty argument list when no injections are defined', async () => {
    ...
    const args = await runtime.resolveInjections(undefined);
