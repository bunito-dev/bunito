...
@LoggerTransport({
  injects: [PrettyTransformConfig],
})
export class PrettyTransform implements LoggerTransport {
  readonly NAME = 'pretty';

  constructor(private readonly config: ResolveConfig<typeof PrettyTransformConfig>) {}

  format(options: LogRecord): string {
